...
namespace IO.Swagger.Api
{
    ...
    public interface ICountriesApi
    {
        ...
        /// <returns>PbxCountryCollectionResponse</returns>
    ...
    public class CountriesApi : ICountriesApi
    {
        ...
        /// <param name="expand">Expand related entities</param>
        /// <returns>PbxCountryCollectionResponse</returns>
        public PbxCountryCollectionResponse ListCountry (int? top, int? skip, string search, string filter, bool? count, List<string> orderby, List<string> select, string expand)
        {
 ...
 if (expand != null) queryParams.Add("$expand", ApiClient.ParameterToString(expand)); // query parameter
            ...
            if (((int)response.StatusCode) >= 400)
                throw new ApiException ((int)response.StatusCode, "Error calling ListCountry: " + response.Content, response.Content);
            else if (((int)response.StatusCode) == 0)
                throw new ApiException ((int)response.StatusCode, "Error calling ListCountry: " + response.ErrorMessage, response.ErrorMessage);
    
            return (PbxCountryCollectionResponse) ApiClient.Deserialize(response.Content, typeof(PbxCountryCollectionResponse), response.Headers);
        }
